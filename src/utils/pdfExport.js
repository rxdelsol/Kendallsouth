export async function exportDashboardPDF(rows){
  ...
  doc.setFontSize(14); doc.text('Kendall South Medical Center', margin+70,36); doc.setFontSize(10); doc.text('Generated: '+nowStr, doc.internal.pageSize.getWidth()-margin-200,36);
...
function addFooter(doc,now){ const page=doc.getNumberOfPages(); const w=doc.internal.pageSize.getWidth(); const h=doc.internal.pageSize.getHeight(); const footer=`Kendall South Medical Center | Generated on ${now.toLocaleString()}`; doc.setFontSize(9); doc.text(footer,40,h-30); doc.text(`Page ${page} of ${page}`, w-120, h-30); }
