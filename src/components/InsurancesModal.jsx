...
export default function InsurancesModal({ open, onClose }){
  ...
  const [name, setName] = useState(''); const [expiration, setExpiration] = useState(''); const [notes, setNotes] = useState('')
  ...
  useEffect(()=>{ if(!insList.includes(active)) setActive(insList[0]||'') },[insList])
                ...
                {(insList||[]).map(ins=> (
                  <div className="ks-card" key={ins}>
                    ...
                    <div className="ks-card-body">
                      <ul>{(doctorsByIns[ins]||[]).map((d,idx)=>(<li key={idx} style={{display:'flex', justifyContent:'space-between', padding:'6px 4px'}}><div><div style={{fontWeight:700}}>{d.name}</div><div style={{fontSize:12, color:'#9aa4b2'}}>{d.expiration?d.expiration:'No expiration'}</div><div style={{fontSize:12, color:'#9aa4b2'}}>{d.notes}</div></div><button onClick={()=>{ const copy={...doctorsByIns}; copy[ins].splice(idx,1); setDoctorsByIns(copy); }}>✕</button></li>))}</ul>
                      <div style={{display:'flex', gap:8, marginTop:8, alignItems:'center'}}>
                        <input placeholder="Doctor name..." value={active===ins?name:''} onChange={e=>setName(e.target.value)} style={{flex:1}} />
                        <input type="date" value={active===ins?expiration:''} onChange={e=>setExpiration(e.target.value)} />
                        <input placeholder="Notes" value={active===ins?notes:''} onChange={e=>setNotes(e.target.value)} />
                        <button onClick={()=>{ if(!name.trim()) return; setDoctorsByIns(prev=>(({...prev, [ins]: [...(prev[ins]||[]), { name: name.trim(), expiration: expiration||null, notes: notes||'' }] }))); setName(''); setExpiration(''); setNotes('') }}>Add</button>
                      </div>
                    </div>
                  </div>
                ))}
