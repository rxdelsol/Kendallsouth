...
const fmt = (iso) =>
  iso ? new Date(String(iso).slice(0, 10) + "T00:00:00").toLocaleDateString("es-US", { day: "2-digit", month: "short", year: "numeric" }) : null;
      ...
      <div className="rec" onClick={(e) => e.stopPropagation()}>
        <div className="rec-top">
          <div style={{ minWidth: 0 }}>
            <span className="eyebrow">Expediente del proveedor</span>
            <h2 className="rec-name">{doctor?.name}</h2>
            <p className="rec-role">{doctor?.taxonomy || "Sin taxonomía registrada"}</p>
            <div className="rec-facts">
              <div className="rec-fact"><span className="eyebrow">NPI</span><span className="v">{doctor?.npi || "—"}</span></div>
              <div className="rec-fact"><span className="eyebrow">Licencia</span><span className="v">{doctor?.license || "—"}</span></div>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 10 }}>
            <button className="rec-close" onClick={onClose}>Cerrar</button>
            <div className={`rec-stamp s-${t}`}>
              <span className="n">{licDays === null ? "—" : licDays}</span>
              <span className="u">{licDays === null ? "sin licencia cargada" : "días de licencia"}</span>
            </div>
          </div>
        </div>
        ...
        <section className="sheet">
          <div className="sechead">
            ...
            <span>La licencia de Florida y la revalidación de Medicare se cargan solas</span>
          </div>
            ...
            {creds.map((c) => {
              ...
              return (
                <div className="rec-row" key={c.key}>
                  ...
                  {c.days === null
                    ? <div className="rec-when none">pendiente de cargar</div>
                    : <div className="rec-when"><b>{fmt(c.date)}</b><small>{c.days} días</small></div>}
                </div>
              );
            })}
          </div>
        ...
        <section className="sheet">
          <div className="sechead">
            <span className="eyebrow">Seguros aceptados</span>
            <span>{rows === null ? "cargando…" : `${cobertura.activos} de ${cobertura.total} contratos activos`}</span>
          </div>
          {rows === null ? (
            <p className="rec-empty">Consultando los contratos…</p>
          ) : cobertura.total === 0 ? (
            <p className="rec-empty">Sin contratos cargados para este proveedor.</p>
          ) : (
              ...
              {LINEAS.map((k) => {
                ...
                return (
                  <div className="rec-linea" key={k}>
                    ...
                    ) : <p className="rec-none">Ninguno activo en esta línea.</p>}
                    {g.fuera.length ? (
                      <div className="rec-chips">
        ...
        {faltantes.length ? (
          <section className="sheet">
            <div className="sechead">
              <span className="eyebrow">Dónde falta contratar</span>
              <span>enlaces de alta verificados</span>
            </div>
            <table className="rec-apply">
              <thead><tr><th>Aseguradora</th><th>Cómo se solicita</th><th></th></tr></thead>
              <tbody>
                {faltantes.map((p) => (
                  <tr key={p.label}>
                    ...
                    <td style={{ textAlign: "right" }}>
                      <a className="rec-link" href={p.url} target="_blank" rel="noopener noreferrer">Aplicar ↗</a>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="rec-foot">
          <span>Licencia verificada en el padrón de MQA · contratos desde tu sistema</span>
          <button onClick={() => { onClose(); onEdit && onEdit(doctor); }}>Editar fechas</button>
        </div>
      </div>
    </div>
  );
